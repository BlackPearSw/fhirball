...
function getComplexTypeMatch(path, value, index) {
    var op = {
        type: 'match',
        def: { $or: []}
    };
    index.forEach(function (child) {
        op.def.$or.push(getCondition(parameters.getPathInResource(path + child), {$regex: '^' + value.text}));
    });
    ...
    string: {
        ...
        Address: {
            ...
            getMatch: function (path, value) {
                return getComplexTypeMatch(path, value, config.string.Address.index)
            },
            modifier: {
                exact: {
                    getMatch: function (path, value) {
        ...
        HumanName: {
            index: ['.family', '.given'],
            getMatch: function (path, value) {
                return getComplexTypeMatch(path, value, config.string.HumanName.index)
            },
            modifier: {
                exact: {
                    getMatch: function (path, value) {
