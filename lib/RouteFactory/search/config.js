...
function getCodeMatch(path, value){
    var op = {
        type: 'match',
        def: {}
    };

    op.def[parameters.getPathInResource(path)] = value.text;
...
function getDateMatch(path, value, isMetaDataSearch) {
    var op = {
        type: 'match',
        def: {}
    };

    var dbPath = isMetaDataSearch ? path : parameters.getPathInResource(path);
    var dateValue = isMetaDataSearch ? new Date(value.text): value.text;

    if (value.comparator) {
        var inequality = {};
        inequality[value.comparator.db] = dateValue;
        op.def[dbPath] = inequality;
    }
    else {
        op.def[dbPath] = isMetaDataSearch ? {$eq: dateValue} : {$regex: '^' + dateValue};
    }
