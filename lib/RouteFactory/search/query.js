...
function mapParamToDbPath(param) {
    return param.document.path.map(function (path) {
        return mapDocumentPathToDbPath(path);
    });
}
...
operationBuilder['search|string|string'] = function (field, searchParams) {
    ...
    ops = [];
    if (param.document.path.length > 1) {
        //TODO:
        ...
        var op = {
            type: 'filter',
            def: filterForString(field, param.document.path[0])
        };
        ops.push(op);
    }
...
function searchStringVsComplexType(field, searchParams, children) {
    var param = conditionFuncs.getSearchParam(searchParams, field.key);
    if (!param) {
        return [];
    }
    ...
    if (param.document.path.length > 1) {
        ...
        children.forEach(function (child) {
            op.def.$or.push(filterForString(field, param.document.path + '.' + child));
        });
        ops.push(op);
    }
...
operationBuilder['search|string|Address'] = function (field, searchParams) {
    return searchStringVsComplexType(field, searchParams, ['line', 'city', 'state', 'zip', 'country']);
};

operationBuilder['search|string|HumanName'] = function (field, searchParams) {
    return searchStringVsComplexType(field, searchParams, ['family', 'given']);
};

function searchTokenVsCodeableConcept(field, searchParams) {
    var param = conditionFuncs.getSearchParam(searchParams, field.key);
    if (!param) {
        return [];
    }
    ...
    if (param.document.path.length > 1) {
        ...
        if (field.key.modifier === 'text') {
            var dbPathForText = conditionFuncs.getDbPath(param.document.path[0] + '.text');
            filter[dbPathForText] = {$regex: '^' + field.value.text};
        }
        else {
            var dbPathForCode = conditionFuncs.getDbPath(param.document.path[0] + '.coding.code');
            filter[dbPathForCode] = field.value.text;

            if (field.value.namespace) {
                var dbPathForCode = conditionFuncs.getDbPath(param.document.path[0] + '.coding.system');
...
operationBuilder['search|token|CodeableConcept'] = function (field, searchParams) {
    return searchTokenVsCodeableConcept(field, searchParams);
};

function searchTokenVsIdentifier(field, searchParams) {
    var param = conditionFuncs.getSearchParam(searchParams, field.key);
    if (!param) {
        return [];
    }
    ...
    if (param.document.path.length > 1) {
        ...
        if (field.key.modifier === 'text') {
            var dbPathForText = conditionFuncs.getDbPath(param.document.path[0] + '.label');
            filter[dbPathForText] = {$regex: '^' + field.value.text};
        }
        else {
            var dbPathForCode = conditionFuncs.getDbPath(param.document.path[0] + '.value');
            filter[dbPathForCode] = field.value.text;

            if (field.value.namespace) {
                var dbPathForCode = conditionFuncs.getDbPath(param.document.path[0] + '.system');
                filter[dbPathForCode] = field.value.namespace === 'NULL_NAMESPACE' ? null : field.value.namespace;
            }
        }
...
operationBuilder['search|token|Identifier'] = function (field, searchParams) {
    return searchTokenVsIdentifier(field, searchParams);
};

operationBuilder['search|token|boolean'] = function (field, searchParams) {
    var param = conditionFuncs.getSearchParam(searchParams, field.key);
    if (!param) {
        return [];
    }
    ...
    if (param.document.path.length > 1) {
        ...
        var dbPath = conditionFuncs.getDbPath(param.document.path[0]);
...
operationBuilder['search|date|date'] = function (field, searchParams) {
    var param = conditionFuncs.getSearchParam(searchParams, field.key);
    if (!param) {
        return [];
    }
...
operationBuilder['search'] = function (field, searchParams) {
    ...
    var fn = operationBuilder['search|' + param.type + '|' + param.document.contentType];
    if (!fn) {
        return [];
    }
