...
operationBuilder['_id'] = function (field) {
    return [
        {
            type: 'match',
            def: {'_id': new mongoose.Types.ObjectId(field.value.text)}
        }
    ];
};
...
operationBuilder['_lastUpdated'] = function (field) {
    var def = config['date']['date'];
    return def.getMatch('meta.lastUpdated', field.value, true);
};
