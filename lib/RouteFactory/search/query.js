...
function mapParamToDbPath(param) {
    return mapDocumentPathToDbPath(param.document.path);
}

var operationBuilder = [];

operationBuilder['_id'] = function (field, searchParams) {
    var path = '_id';
...
operationBuilder['search|string|string'] = function (field, searchParams) {
    ...
    var op = {
        type: 'filter',
        def: filterForString(field, param.document.path)
    };
    ops.push(op);
...
function searchTokenVsCodeableConcept(field, searchParams) {
    ...
    if (field.key.modifier === 'text') {
        var dbPathForText = conditionFuncs.getDbPath(param.document.path + '.text');
        filter[dbPathForText] = {$regex: '^' + field.value.text};
    }
    else {
        var dbPathForCode = conditionFuncs.getDbPath(param.document.path + '.coding.code');
        filter[dbPathForCode] = field.value.text;

        if (field.value.namespace) {
            var dbPathForCode = conditionFuncs.getDbPath(param.document.path + '.coding.system');
...
function searchTokenVsIdentifier(field, searchParams) {
    ...
    if (field.key.modifier === 'text') {
        var dbPathForText = conditionFuncs.getDbPath(param.document.path + '.label');
        filter[dbPathForText] = {$regex: '^' + field.value.text};
    }
    else {
        var dbPathForCode = conditionFuncs.getDbPath(param.document.path + '.value');
        filter[dbPathForCode] = field.value.text;

        if (field.value.namespace) {
            var dbPathForCode = conditionFuncs.getDbPath(param.document.path + '.system');
            filter[dbPathForCode] = field.value.namespace === 'NULL_NAMESPACE' ? null : field.value.namespace;
        }
    }
