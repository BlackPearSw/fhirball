...
function makeIndex(extension, options) {
    ...
    if (options.index) {
        ...
        def.forEach(function (item) {
            var index = {'path': extension.path, 'enable': extension.index, 'columns':{}};
...
function makeDefaultIndexes() {
    return [
        {'path': '_id', 'enable': true, 'columns': {'_id': 1}},
        {'path': 'tags', 'enable': true, 'columns': {'tags.term': 1, 'tags.scheme': 1}}
    ];
}
...
function decoratePojoWithSearchProperties(pojo, searchParam) {
    ...
    searchParam
        ...
        .forEach(function (doc) {
            ...
            if (value) {
                jsoq(pojo).put(searchPath, value.toUpperCase());
            }
        });
