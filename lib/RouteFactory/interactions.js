...
exports.vread = function (model, contentType, searchParam, auditModel) {
    return function (req, res) {
        ...
        auditModel.findOne(criteria, function (err, doc) {
            if (err) return res.status(400).end();
            if (!doc) return res.status(404).end();
...
exports.update = function (model, contentType, searchParam, auditModel) {
    return function (req, res) {
        ...
        model.findOneAndUpdateWithOptimisticConcurrencyCheck(obj, function (err, doc) {
            ...
            if (auditModel) {
                var auditObj = auditModel(doc.toObject());
                auditObj._id = new mongoose.Types.ObjectId();
                auditObj.save(function (err) {
                    if (err) {
                        console.log(err);
                    }
                    //TODO: What do we do when err saving to audit?
                });
            }
    ...
    return function (req, res) {
        ...
        model.findOneAndRemove(criteria, function (err, doc) {
            ...
            if (auditModel) {
                ...
                pojo.deleted = pojo.meta.lastUpdated;
                pojo.resource = {resourceType: pojo.resource.resourceType};

                auditModel(pojo).save(function (err) {
                    if (err) {
                        console.log(err);
                    }
                    //TODO: What do we do when err saving to audit?
...
exports.history = function (model, contentType, searchParam, auditModel) {
    return function (req, res) {
        if (!req.params.id) {
            return res.status(500).end();
        }
