...
var Router = function (options) {
    ...
    function parseResource(resource, callback) {
        ...
        function parseOperation(model, callback) {
            ...
            resource.operation.sort().reverse();
            ...
            async.map(resource.operation, function (operation, callback) {
                if (operation.code === 'search-type') {
                    ...
                    if (operation.code === 'create') {
                        api.post('/' + resource.type + '/:id/_tags/_delete', routeFactory.makeDeleteTagsForInstance(model));
                        api.post('/' + resource.type + '/:id/_tags', routeFactory.makeCreateTagsForInstance(model));
